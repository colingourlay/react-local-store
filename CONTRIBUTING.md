...
2. Git clone the `react-local-store` [repository](https://github.com/colingourlay/react-local-store).
...
The build process uses [`microbundle`](https://github.com/developit/microbundle) to create two artifacts from the `src/react-local-store.ts` entry point:

- `dist/react-local-store.js` for `require()`-ing with CommonJS
- `dist/react-local-store.m.js` for `import`-ing as an ECMAScript Module
...
By contributing to `react-local-store`, you agree that your contributions will be licensed under [The Unlicense](LICENSE).
